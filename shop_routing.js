...
http.createServer((req,res)=>{
    ...
    if(url_parse.pathname === "/" || url_parse.pathname==="/index.html"){
        fs.readFile("public/index.html","utf-8",(e,index)=>{
            // readFile 성공하면 문자열(text)의 형태로 사용가능
            res.end(index);
        });
    }else if(url_parse.pathname === "/member/list.do"){
        fs.readFile("public/memList.html","utf-8",(e,memList)=>{
        ...
        const conn=mysql.createConnection(login_info);
        conn.connect((e)=>{
            conn.query("SELECT * FROM MEMBER WHERE ID=?",[url_parse.query["id"]],
            (e,mem)=>{
                res.setHeader("Content-Type","application/json;charset=UTF-8");
        ...
        req.on("end",()=>{
            ...
            conn.connect((e)=>{
                ...
                [form_data["name"],form_data["phone"],form_data["birth"],form_data["id"]],(e,result)=>{
        ...
        conn.connect((e)=>{
            conn.query("DELETE FROM MEMBER WHERE ID=?",[url_parse.query["id"]],(e,result)=>{
                const delete_obj={delete:0, msg:""};
                if(e){
                    delete_obj.delete=-1;
                    delete_obj.msg="에러:"+e.message;
                }else{
        ...
        req.on("end",()=>{
            ...
            conn.connect((e)=>{
                ...
                [form.id,form.name,form.phone,form.birth],(e,result)=>{
                    const result_obj={result:0,msg:""};
                    if(e){
                        result_obj.result=-1;
                        result_obj.msg="에러"+e.message;
        ...
        // on("data") : buffer에 data를 읽는 중 => on("end") : data가 끝나는 지점
    }else{
